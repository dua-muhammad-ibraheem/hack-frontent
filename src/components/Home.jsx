import { Link } from "react-router-dom";

const Home = () => {
  return (
    <main>
      {/* Hero */}
      <section className="bg-white">
        <div className="mx-auto grid min-h-[calc(100vh-64px)] max-w-7xl items-center gap-14 px-6 py-20 lg:grid-cols-2 lg:py-24">

          <div>
            <div className="mb-6 inline-flex rounded-full bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-600">
              Find the Right Worker for Your Problem
            </div>

            <h1 className="max-w-2xl text-5xl font-bold leading-tight tracking-tight text-gray-900 md:text-6xl">
              Get your problems solved
              <span className="text-blue-600"> by the right professional.</span>
            </h1>

            <p className="mt-6 max-w-xl text-lg leading-8 text-gray-600">
              Submit your complaint, choose a service category, find the
              right worker, and track your request from start to completion
              in one simple place.
            </p>

            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                to="/signup"
                className="rounded-xl bg-blue-600 px-7 py-3.5 font-semibold text-white transition hover:bg-blue-700"
              >
                Get Started
              </Link>

              <a
                href="#how-it-works"
                className="rounded-xl border border-gray-300 px-7 py-3.5 font-semibold text-gray-800 transition hover:bg-gray-50"
              >
                How It Works
              </a>
            </div>

            <div className="mt-10 flex flex-wrap gap-8 text-sm text-gray-600">
              <div>
                <span className="font-bold text-gray-900">Easy</span>
                <p>Submit your complaint</p>
              </div>

              <div>
                <span className="font-bold text-gray-900">Choose</span>
                <p>Select the right worker</p>
              </div>

              <div>
                <span className="font-bold text-gray-900">Track</span>
                <p>Follow your request</p>
              </div>
            </div>
          </div>

          {/* Dashboard Preview */}
          <div className="relative">
            <div className="rounded-3xl border border-gray-200 bg-gray-50 p-4 shadow-2xl">
              <div className="rounded-2xl bg-white p-6">

                <div className="flex items-center justify-between border-b border-gray-100 pb-5">
                  <div>
                    <p className="text-sm text-gray-500">
                      Service Dashboard
                    </p>

                    <h2 className="mt-1 text-xl font-bold text-gray-900">
                      My Requests
                    </h2>
                  </div>

                  <div className="rounded-lg bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-600">
                    Active
                  </div>
                </div>

                <div className="mt-6 grid grid-cols-3 gap-3">
                  <div className="rounded-xl bg-gray-50 p-4">
                    <p className="text-xs text-gray-500">Pending</p>
                    <p className="mt-1 text-2xl font-bold text-gray-900">12</p>
                  </div>

                  <div className="rounded-xl bg-gray-50 p-4">
                    <p className="text-xs text-gray-500">In Progress</p>
                    <p className="mt-1 text-2xl font-bold text-gray-900">08</p>
                  </div>

                  <div className="rounded-xl bg-gray-50 p-4">
                    <p className="text-xs text-gray-500">Completed</p>
                    <p className="mt-1 text-2xl font-bold text-gray-900">24</p>
                  </div>
                </div>

                <div className="mt-6 space-y-3">
                  <div className="flex items-center justify-between rounded-xl border border-gray-100 p-4">
                    <div>
                      <p className="text-sm font-semibold text-gray-900">
                        Pipeline leakage
                      </p>

                      <p className="mt-1 text-xs text-gray-500">
                        Plumbing · Worker Assigned
                      </p>
                    </div>

                    <span className="rounded-full bg-yellow-50 px-3 py-1 text-xs font-semibold text-yellow-700">
                      Pending
                    </span>
                  </div>

                  <div className="flex items-center justify-between rounded-xl border border-gray-100 p-4">
                    <div>
                      <p className="text-sm font-semibold text-gray-900">
                        Electrical wiring issue
                      </p>

                      <p className="mt-1 text-xs text-gray-500">
                        Electrical · Worker Accepted
                      </p>
                    </div>

                    <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-600">
                      In Progress
                    </span>
                  </div>

                  <div className="flex items-center justify-between rounded-xl border border-gray-100 p-4">
                    <div>
                      <p className="text-sm font-semibold text-gray-900">
                        AC maintenance
                      </p>

                      <p className="mt-1 text-xs text-gray-500">
                        AC Repair · Service Completed
                      </p>
                    </div>

                    <span className="rounded-full bg-green-50 px-3 py-1 text-xs font-semibold text-green-600">
                      Completed
                    </span>
                  </div>
                </div>

              </div>
            </div>
          </div>

        </div>
      </section>

      {/* Features */}
      <section
        id="how-it-works"
        className="border-t border-gray-100 bg-gray-50 px-6 py-20"
      >
        <div className="mx-auto max-w-7xl">

          <div className="max-w-2xl">
            <p className="text-sm font-semibold text-blue-600">
              Simple and easy
            </p>

            <h2 className="mt-2 text-3xl font-bold tracking-tight text-gray-900 md:text-4xl">
              How our service works
            </h2>

            <p className="mt-4 leading-7 text-gray-600">
              From submitting your complaint to getting your problem solved,
              everything stays organized in one simple workflow.
            </p>
          </div>

          <div className="mt-12 grid gap-6 md:grid-cols-3">

            <div className="rounded-2xl border border-gray-200 bg-white p-7">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-100 font-bold text-blue-600">
                01
              </div>

              <h3 className="mt-5 text-lg font-semibold text-gray-900">
                Submit Your Complaint
              </h3>

              <p className="mt-2 leading-6 text-gray-600">
                Describe your problem and select the service category that
                best matches your complaint.
              </p>
            </div>

            <div className="rounded-2xl border border-gray-200 bg-white p-7">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-100 font-bold text-blue-600">
                02
              </div>

              <h3 className="mt-5 text-lg font-semibold text-gray-900">
                Choose a Worker
              </h3>

              <p className="mt-2 leading-6 text-gray-600">
                View available workers for your selected category and choose
                the professional you want to handle your request.
              </p>
            </div>

            <div className="rounded-2xl border border-gray-200 bg-white p-7">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-100 font-bold text-blue-600">
                03
              </div>

              <h3 className="mt-5 text-lg font-semibold text-gray-900">
                Track and Review
              </h3>

              <p className="mt-2 leading-6 text-gray-600">
                Track your request while the worker accepts and completes the
                job, then share your experience with a rating and review.
              </p>
            </div>

          </div>
        </div>
      </section>
    </main>
  );
};

export default Home;